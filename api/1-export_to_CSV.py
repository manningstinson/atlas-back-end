...
def export_to_csv(user_id):
    """
    Export tasks belonging to the
    specified user to a CSV file.

    Args:
        user_id (int): The ID of the user
        whose tasks are to be exported.

    Returns:
        None
    """
    # Ensure that todos.json file exists
    # in the current directory or provide correct path
    ...
    with open(csv_filename, 'w', newline='') as csv_file:
        ...
        for task in tasks:
            writer.writerow({
                'USER_ID': task['userId'],
                # Assuming username is same for all tasks
